...
class XCostPoster
  ...
  def parse_costs_file
    ...
    feature_sections.each do |section|
      if section.match(/### ([^\n]+)\n\*\*Cost\*\*: \$([0-9.]+)/)
        ...
        duration = nil
        if section.match(/\*\*Duration\*\*: ([^\n]+)/)
          duration = $1.strip
        end
        
        # Extract features
        features_text = ""
        if section.match(/\*\*Features Implemented\*\*:\n(.*?)(?=\n\*\*Key Deliverables|### |## |\z)/m)
    ...
    # Parse totals
    if content.match(/- \*\*Total Cost\*\*: \$([0-9.]+)/)
      totals[:total_cost] = $1.to_f
    end
    
    if content.match(/- \*\*Total Features\*\*: ([0-9]+)/)
  ...
  def generate_main_tweet(feature_name, feature_data, totals)
    ...
    tweet = "🤖 I just had Claude AI write production #{feature_name.downcase} for a Ruby gem\n\n"
    tweet += "💰 Cost: $#{sprintf('%.2f', cost)}"
    
    if duration
      # Extract just the wall time for simplicity
      if duration.match(/(\d+m \d+s).*wall/)
        tweet += " | ⏱️ #{$1}"
      end
    end
    ...
    tweet += "This is wild. Thread with technical details ↓"
